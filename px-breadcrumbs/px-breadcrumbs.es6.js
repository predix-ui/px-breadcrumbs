(function (window) {
  Polymer({
    ...
    behaviors: [PxAppBehavior.AssetGraph, PxAppBehavior.AssetSelectable, Polymer.IronResizableBehavior],
    ...
    observers: ['_getDisplayMode(_ulWidth, _selectedItemPath, _breadcrumbsObj)'],
    ...
    _onResize: function _onResize() {
      ...
      this.debounce('windowResize', function () {
        window.requestAnimationFrame(function () {
          var breadcrumbsContainer = Polymer.dom(_this.root).querySelector('.container'),
          ...
          _this.set('_ulWidth', bcUlContainerRect.width);
        });
      }, 50);
    },
    _getBreadcrumbsObj: function _getBreadcrumbsObj() {
    ...
    */
    _getDisplayMode: function _getDisplayMode() {
    ...
    */
    _createArrayWithOverflow: function _createArrayWithOverflow(strArray, _ulWidth, breadcrumbsObj) {
      ...
      overflowObj.children.forEach(function (child) {
     ...
     */
    _isNotFirstItemInData: function _isNotFirstItemInData(index) {
      return index !== 0;
    },
     ...
     */
    _doesItemHaveSiblings: function _doesItemHaveSiblings(itemInPath) {
     ...
     */
    _dropdownTap: function _dropdownTap(evt) {
      var newSelectItem = {};
     ...
     */
    _changePathFromClick: function _changePathFromClick(item) {
      this.select(item);
    ...
    */
    _onPathTap: function _onPathTap(evt) {
      ...
      if (this._doesItemHaveSiblings(dataItem) || isClickedItemOverflow) {
        ...
        var siblingsCopy = siblings.map(function (sibling) {
     ...
     */
    _isLabel: function _isLabel(item, clickOnlyMode) {
     ...
     */
    _isDropdown: function _isDropdown(item, clickOnlyMode) {
     ...
     */
    _isOverflow: function _isOverflow(item) {
      return item.label === "...";
    },
     ...
     */
    _getSeparatorSize: function _getSeparatorSize(clickOnlyMode) {
      return this.clickOnlyMode ? 'small' : 'large';
    }
  });

  var Breadcrumbs = function () {
    function Breadcrumbs(breadcrumbEl, graph, clickOnlyMode) {
      var breadcrumbs = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : [];
    ...
    babelHelpers.createClass(Breadcrumbs, [{
       ...
       */
      value: function _addToWeakMap(item) {
        var cachedItem = this.map.get(item) || null;
        if (!cachedItem) {
      ...
      key: '_preShortenItems',
      value: function _preShortenItems(items) {
      ...
      value: function _getShortenedText(item) {
        var cachedItem = this.map.get(item) || {};
      ...
      key: '_sizeOfIndividualFullItem',
      value: function _sizeOfIndividualFullItem(item) {
        var cachedItem = this.map.get(item) || {};
      ...
      key: '_sizeOfIndividualShortItem',
      value: function _sizeOfIndividualShortItem(item) {
        var cachedItem = this.map.get(item) || {};
      ...
      key: '_calculateSizeOfBreadcrumbs',
      value: function _calculateSizeOfBreadcrumbs(strArray) {
        var useFullSize = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : true;
      ...
      key: '_createCanvas',
      value: function _createCanvas(breadcrumbEl) {
      ...
      key: 'sizeOfAllShortenedItemsExcludingLastItem',
      get: function get() {
      ...
      get: function get() {
        ...
        this.__shortenedItems = this.__shortenedItems || this.breadcrumbs.map(function (item) {
          var wrapper = {};
          wrapper.source = item;
          wrapper.isTruncated = true;
          wrapper.label = _this2._getShortenedText(item);
      ...
      key: 'allShortenedItemsExcludingLast',
      get: function get() {
